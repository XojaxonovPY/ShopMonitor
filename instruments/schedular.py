...
async def update_product_price():
    async with AsyncSessionLocal() as session:
        ...
        products: list[Product] = await Product.get_all(session)
        for product in products:
            if product.url.startswith("https://clickbrandshop.robostore.uz"):
                driver.get(product.url)
                product_data = await get_click_market(wait, product.url)

            elif product.url.startswith("https://uzum.uz/uz"):
                driver.get(product.url)
                product_data = await get_grapes_market(wait, product.url)
